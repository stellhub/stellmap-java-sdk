...
final class NettyHttpTransport {
    ...
    private NettyHttpResponse executeRequest(
                ...
                future -> {
                    if (!future.isSuccess()) {
                        responseFuture.completeExceptionally(
                                new StellMapTransportException("Failed to call StarMap HTTP API", future.cause()));
                    }
                });
        ...
        try {
            ...
            throw new StellMapTransportException("Interrupted while calling StarMap HTTP API", e);
    ...
    private ScheduledFuture<?> scheduleTimeout(
        ...
        return eventLoopGroup
                ...
                .schedule(
                        () -> {
                            if (future.completeExceptionally(new StellMapTransportException(message))) {
    ...
    private static final class AggregatedResponseHandler
        ...
        public void channelInactive(ChannelHandlerContext context) {
            if (!responseReceived) {
                responseFuture.completeExceptionally(
                        new StellMapTransportException("Connection closed before receiving StarMap response"));
            }
        }
        ...
        public void exceptionCaught(ChannelHandlerContext context, Throwable cause) {
            responseFuture.completeExceptionally(
                    new StellMapTransportException("Failed to call StarMap HTTP API", cause));
            context.close();
        }
    }
