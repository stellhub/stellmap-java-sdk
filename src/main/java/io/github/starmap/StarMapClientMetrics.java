...
final class StarMapClientMetrics {
    ...
    private static final AttributeKey<Boolean> ATTR_SUCCESS = AttributeKey.booleanKey("starmap.success");
    private static final AttributeKey<String> ATTR_EVENT_TYPE = AttributeKey.stringKey("starmap.event_type");
    private static final AttributeKey<Long> ATTR_ATTEMPT = AttributeKey.longKey("starmap.reconnect_attempt");
    private static final AttributeKey<String> ATTR_ERROR = AttributeKey.stringKey("starmap.error_type");
    private static final AttributeKey<Boolean> ATTR_SCHEDULED = AttributeKey.booleanKey("starmap.scheduled");
    ...
    StarMapClientMetrics(OpenTelemetry openTelemetry, String instrumentationName, String instrumentationVersion) {
        Meter meter = openTelemetry.getMeterProvider()
                .meterBuilder(instrumentationName)
                .setInstrumentationVersion(instrumentationVersion)
                .build();
        this.requestCounter = meter.counterBuilder("starmap_client_request_total")
                .setDescription("Total StarMap client requests.")
                .build();
        this.requestLatencyMillis = meter.histogramBuilder("starmap_client_request_duration_ms")
                .setUnit("ms")
                .setDescription("StarMap client request duration in milliseconds.")
                .build();
        this.watchEventCounter = meter.counterBuilder("starmap_client_watch_event_total")
                .setDescription("Total StarMap watch events.")
                .build();
        this.watchReconnectCounter = meter.counterBuilder("starmap_client_watch_reconnect_total")
                .setDescription("Total StarMap watch reconnect attempts.")
                .build();
        this.activeWatchSubscriptions = meter.upDownCounterBuilder("starmap_client_watch_active")
                .setDescription("Current active StarMap watch subscriptions.")
                .build();
        this.activeDirectoryServices = meter.upDownCounterBuilder("starmap_client_directory_service_active")
                .setDescription("Current active services held in local watch caches.")
                .build();
        this.activeDirectoryInstances = meter.upDownCounterBuilder("starmap_client_directory_instance_active")
                .setDescription("Current active instances held in local watch caches.")
                .build();
        this.autoDeregisterCounter = meter.counterBuilder("starmap_client_auto_deregister_total")
                .setDescription("Total StarMap auto deregister attempts on close.")
                .build();
        this.heartbeatCounter = meter.counterBuilder("starmap_client_heartbeat_total")
                .setDescription("Total StarMap heartbeats.")
                .build();
        this.heartbeatLatencyMillis = meter.histogramBuilder("starmap_client_heartbeat_duration_ms")
                .setUnit("ms")
                .setDescription("StarMap heartbeat duration in milliseconds.")
                .build();
        this.activeHeartbeatSchedules = meter.upDownCounterBuilder("starmap_client_heartbeat_schedule_active")
                .setDescription("Current active StarMap scheduled heartbeats.")
                .build();
        this.activeRegistrations = meter.upDownCounterBuilder("starmap_client_registration_active")
                .setDescription("Current active tracked StarMap registrations.")
                .build();
    }

    void recordRequest(String method, String path, int statusCode, long durationNanos, boolean success) {
    ...
    void recordWatchEvent(String eventType) {
        watchEventCounter.add(1L, Attributes.of(
                ATTR_EVENT_TYPE, eventType == null || eventType.isBlank() ? "unknown" : eventType.trim().toUpperCase()
        ));
    }

    void recordWatchReconnect(int attempt, Throwable failure) {
        watchReconnectCounter.add(1L, Attributes.builder()
